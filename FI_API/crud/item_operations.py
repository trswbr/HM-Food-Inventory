...
async def crud_get_all_items(
    ...
    # Find items
    cursor_item = conn[item_collection_name].find(filter=filter_query, skip=skip, limit=limit).sort(*sort)
...
async def crud_get_item(
    ...
    """
    response_item = await conn[item_collection_name].find_one({"_id": item_id})
...
async def crud_create_item(
    ...
    response = await conn[item_collection_name].insert_one(Item(
        _id=item_id,
...
async def crud_update_item(
    ...
    """
    response = await conn[item_collection_name].find_one_and_update(
        filter={"_id": item_id},
...
async def _update_status_item(
    ...
    """
    response = await conn[item_collection_name].update_one(
        filter={"_id": item_id},
